  ...
  <div class="articleContainer">
    ...
    <el-card class="container" shadow="never">
      <div class="header">
        ...
        <h2 style="text-align:center;font-size:20px">{{article.helpTitle}}</h2>
      </div>
      <div>
        <p style="margin-top:8px;">{{article.helpInformation}}</p>
      </div>
      <div class="size-icon">
      ...
      <el-form :model="commentForm" ref="commentForm" class="mm">
        <el-form-item>
...
export default {
  ...
  methods:{
    async articleData(){
      ...
      const res = await this.getRequest('/help-information/gain/' + this.$route.params.helpId)
    ...
    async commentData(){
      const res = await this.getRequest('/help-comment/gainComment/'+this.$route.params.helpId)
      console.log(res);
    ...
    addComment(){
      this.commentForm.helpId=this.$route.params.helpId
      ...
      if(this.commentForm.commentBody===''){
        ...
        this.$refs.commentForm.validate((valid) => {
          if (valid) {
            this.postRequest('/help-comment/addComment',this.commentForm).then(resp => {
              if(resp) {
                location.reload()
              }
            })
