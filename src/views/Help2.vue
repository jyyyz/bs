<template>
  <div class="helpInformationContainer">
    <el-card
      ...
      :key="item.helpId"
      >
      <div class="helpList" @click="$router.push(`/helpArticle/${item.helpId}`)" @contextmenu.prevent="show()">
        ...
        <el-collapse-transition>
          <el-button v-if="value" @click.stop="show()" type="warning" style="position:absolute;bottom:0px;right:120px">取消</el-button>
        </el-collapse-transition>
        <el-collapse-transition>
          <el-button v-if="value" @click.stop="delet(item.helpId)" type="danger" style="position:absolute;bottom:0px;right:30px">删除</el-button>
        </el-collapse-transition>
      </div>
    </el-card>
    <el-pagination
...
export default {
  ...
  methods:{
    ...
    delet(id){
      this.$confirm('此操作将删除此求助, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.deleteRequest('/help-information/'+id).then(resp=>{
        if(resp){
          location.reload()
        }
        })
    ...
    initHelp(){
      this.getRequest('/help-information/userHelp').then(resp=>{
        if(resp){
          this.help=resp;
          console.log(resp);
          this.getList()
