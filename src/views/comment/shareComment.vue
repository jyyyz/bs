<template>
  <div class="manage">
    <el-dialog
      :title="operateType==='add'?'新增分享评论':'更新分享评论'"
      :visible.sync="isShow"
    >
      <common-form
  ...
  methods:{
    confirm(){
      ...
      if(this.operateType==='add'){
        this.postRequest('/share-comment/add',this.operateForm).then(resp => {
          if(resp) {
            location.reload()
            this.getList()
          }
      })}else{
        this.putRequest('/share-comment/updateComment',this.operateForm).then(resp => {
          if(resp) {
            location.reload()
            this.getList()
          }
    ...
    getList(){
      this.getRequest('/share-comment/gain').then(resp=>{
        if(resp){
          this.tableData=resp;
          this.getData()
        }
    ...
    delUser(row){
      ...
      this.$confirm("是否删除?","提示",{
        ...
        this.deleteRequest('/share-comment/'+row.commentId).then(resp=>{
          if(resp){
            location.reload()
          }
        })
    ...
    search(keyword){
      console.log(keyword);
      this.getRequest('/share-comment/search/'+keyword).then(resp=>{
        if(resp){
          this.tableData=resp
          console.log(keyword);
          this.getData()
