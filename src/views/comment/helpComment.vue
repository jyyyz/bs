<template>
  <div class="manage">
    <el-dialog
      :title="operateType==='add'?'新增求助评论':'更新求助评论'"
      :visible.sync="isShow"
    >
      <common-form
  ...
  methods:{
    confirm(){
      if(this.operateType==='add'){
        this.postRequest('/help-comment/add',this.operateForm).then(resp => {
          if(resp) {
            location.reload()
            this.getList()
          }
      })}else{
        this.putRequest('/help-comment/updateComment',this.operateForm).then(resp => {
          if(resp) {
            location.reload()
            this.getList()
          }
    ...
    getList(){
      this.getRequest('/help-comment/gain').then(resp=>{
        if(resp){
          this.tableData=resp;
          this.getData()
        }
    ...
    delUser(row){
      ...
      this.$confirm("是否删除?","提示",{
        ...
        this.deleteRequest('/help-comment/'+row.commentId).then(resp=>{
          if(resp){
            location.reload()
          }
        })
    ...
    search(keyword){
      console.log(keyword);
      this.getRequest('/help-comment/search/'+keyword).then(resp=>{
        if(resp){
          this.tableData=resp
          console.log(keyword);
          this.getData()
